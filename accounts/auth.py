...
from .models import PmAuthToken, PlayMateUser as User
...
class TokenAuthentication(BaseAuthentication):
    ...
    If successful
    - `request.user` will be a Play mate `User` instance
    ...
    def authenticate_credentials(self, token):
        ...
        for auth_token in PmAuthToken.objects.filter(
        ...
        for other_token in auth_token.user.auth_token_set.all():
            if other_token.digest != auth_token.digest and other_token.expiry:
                if other_token.expiry < timezone.now():
                    other_token.delete()
                    phone_number = other_token.user.get_phone_number()
                    token_expired.send(sender=self.__class__,
                                       phone_number=phone_number, source="other_token")
        if auth_token.expiry is not None:
            if auth_token.expiry < timezone.now():
                phone_number = auth_token.user.get_phone_number()
                auth_token.delete()
                token_expired.send(sender=self.__class__,
                                   phone_number=phone_number, source="auth_token")
                return True
        return False
